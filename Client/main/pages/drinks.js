...
const drinkCartItems = document.getElementById("cart-items");
const drinkPlaceOrderButton = document.getElementById("place-order");
const drinkCartLength = document.getElementById("cart-length");
const drinkCartButton = document.getElementById("cart-button");
let drinkCart = [];
const drinkProducts = [
    {
        id: 1,
...
function displayDrinkItems() {
    ...
    addToCartButtons.forEach((button) => button.addEventListener("click", () => {
        const drinkId = parseInt(button.getAttribute("data-id"));
        addDrinkToCart(drinkId);
        showDrinkMessage("Item added to cart!");
    }));
}
function showDrinkMessage(message) {
...
function addDrinkToCart(drinkId) {
    ...
    if (existingItem) {
        ...
        drinkCart.push(Object.assign(Object.assign({}, drink), { amount: 1 }));
    }
    updateDrinkCart();
    saveDrinkCartToLocalStorage();
}
// Delete from cart
function deleteDrinkFromCart(drinkId) {
    drinkCart = drinkCart.filter((item) => item.id !== drinkId);
    updateDrinkCart();
    saveDrinkCartToLocalStorage();
}
// Update cart UI
function updateDrinkCart() {
    ...
    drinkCart.forEach((item, index) => {
        ...
        li.innerHTML = `<span style="display: inline-block; width: ${maxLength}ch;"> ${index + 1}.
            ...
            .join("")}
                ...
                <span>
                = $${item.price.toFixed(2)} * ${(_a = item.amount) === null || _a === void 0 ? void 0 : _a.toFixed(0)}
                </span>`;
        const headerRow = document.createElement("li");
...
function updateDrinkItemAmount(drinkId, newAmount) {
...
function saveDrinkCartToLocalStorage() {
    localStorage.setItem("drinkCart", JSON.stringify(drinkCart));
}
// Load cart from local storage
function loadDrinkCartFromLocalStorage() {
